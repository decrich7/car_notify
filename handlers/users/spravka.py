...
async def get_spravka(message: types.Message):
    await message.answer(f'Car Notify - это сервис который поможет отслеживать появление новых объявлений'
                         ...
                         'Чтобы начать нажмите кнопку - "Добавить категорию автомобилей 🚗"')


@dp.message_handler(text='/spravka')
async def get_spravka(message: types.Message):
